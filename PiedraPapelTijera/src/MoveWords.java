...
public class MoveWords{
    ...
    public static int checkWinner(String first, String second){
	    ...
	    /*
	     * Primer prueba del juego:
	     * 
	     * return ( second_i ==(first_i +1) % 5||second_i == (first_i + 2) % 5) ? GANA: PIERDE;
	     * si se usa esta se agraga una llave al final.
	    */
	   ...
	   if  ((first_i==0 ) &&
			   ( second_i==1 || second_i==3)) {
		   return GANA ;
		   }
	    if((first_i==1 ) &&
	    		( second_i==2 || second_i==4)) {
	    	return GANA ;
	    	}
	     if((first_i==2 ) &&
	    		 (second_i==0 || second_i==3)){
	    	 return GANA ;
	    	 }
	    if ((first_i==3 ) &&
	    		(second_i==1 || second_i==4)) {
	    	return GANA ;
	    	} 
	    if((first_i==4 ) &&
	    		( second_i==0 || second_i==2  )) {
	    	return GANA ;
	    }
	    else return PIERDE;
	   }
